...
const getAllArticles = expressAsyncHandler(
    async (req: Request<{}, {}, {}, AllArticles>, res: Response) => {
        try {
            const maxLimit = 20;
            const limit = Number(req.query.limit ? (req.query.limit <= maxLimit ? req.query.limit : maxLimit) : maxLimit);
            const page = Number(req.query.page || 1);
            const count = await Article.find().count();
            const articles = await Article.find().sort({ createdAt: req.query.sort || 'desc' }).limit(limit).skip((page - 1) * limit);
            res.status(200).json({
                articles: articles,
                page: page,
                limit: limit,
